...
bool CommandRecorder::loadJSON(const std::string& filePath) {
    ...
    while ((pos = content.find('{', pos)) != std::string::npos) {
        ...
        std::string part; bool isMeta = false;
...
bool CommandRecorder::load(const std::string& filePath) {
    ...
    while (std::getline(ifs, line)) {
        ...
        if (!(iss >> op >> target >> index)) continue;
        RecordedCommand rc; rc.op = op; rc.target = target; rc.index = index; rc.value = 0; rc.hasValue = false; rc.t = 0.0; // texto não tem timestamp
        if (op == "INSERT") {
            if (iss >> value) { rc.value = value; rc.hasValue = true; }
        }
        m_commands.push_back(rc);
    }
    return true;
